...
const completenessFields: CompletenessField[] = [
  ...
  { label: "Yakıt türü", isComplete: (input) => Boolean(input.fuelType) },
  { label: "Vites türü", isComplete: (input) => Boolean(input.transmission) },
  { label: "Kilometre", isComplete: (input) => input.mileage >= 0 },
  { label: "İlan fiyatı", isComplete: (input) => input.price > 0 },
  { label: "Şehir", isComplete: (input) => Boolean(input.city) },
